...
```
- **后端建议实现逻辑**:
  1. 调用 LLM（如 GPT-4, 文心一言等）的 API。
  2. 提示词 (Prompt) 参考：`请从以下文本中提取日程信息，并以 JSON 格式输出包含 title, date, time (HH:mm:ss), location, notes 的结果。当前日期是 2024-01-23。文本内容：{text}`。
  3. 将 LLM 结果封装为 `ApiResponse<Schedule>` 返回。
